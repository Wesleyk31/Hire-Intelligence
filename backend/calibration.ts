...
export function groupReviewedOutcomes<P extends NamedProject>(
  projects: P[],
  ...
  for (const outcome of outcomes) {
    ...
    const project = outcome.projectId
      ? byId.get(outcome.projectId)
      : matches.length === 1
        ? matches[0]
        : undefined;
    if (!project) {
      unmatched.add(outcome.projectId || name);
      continue;
    }
