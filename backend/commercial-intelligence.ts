...
type EventType = 'SHUTDOWN' | 'OUTAGE' | 'RESTART' | 'EXPANSION' | 'MOBILISATION' | 'MAINTENANCE' | 'PROCUREMENT' | 'AWARD' | 'APPROVAL' | 'NEGATIVE';
...
function projectText(project: ProjectIntelligence) {
  return project.records.map(record => `${record.project} ${record.description} ${record.sourceKey}`).join(' ').toLowerCase();
}
...
function eventSignals(project: ProjectIntelligence) {
  ...
  add('RESTART', ['restart', 'recommission', 'reactivat', 'return to production', 'resume operations'], 'Published evidence indicates restart, recommissioning or reactivation activity.', 6);
  add('EXPANSION', ['expansion', 'brownfield', 'capacity increase', 'extension', 'new pit', 'plant upgrade'], 'Published evidence indicates expansion or capacity-growth work.', 5);
...
function actionFor(project: ProjectIntelligence) {
  if (project.stageLabel === 'COMPLETE') return 'Archive from active BDM focus unless new restart evidence appears.';
  if (!project.contractors.length) return 'Identify and verify the delivery contractor from public evidence before outreach.';
  if (project.stageLabel === 'SHUTDOWN' || project.stageLabel === 'MAINTENANCE') return 'Verify work window, contractor and equipment requirement before outreach.';
  if (project.stageLabel === 'AWARDED' || project.stageLabel === 'MOBILISATION') return 'Verify mobilisation timing and actual equipment requirement with the evidence-backed contractor.';
  if (project.stageLabel === 'PROCUREMENT') return 'Track award outcome and identify likely bidders/contractor before mobilisation.';
  if (project.stageLabel === 'APPROVAL') return 'Monitor for procurement, award or early-works evidence.';
...
function evidenceNeeded(project: ProjectIntelligence) {
  ...
  if (!project.equipmentPrediction.classes.length) needs.push('specific work scope for equipment prediction');
  if (project.stageLabel === 'WATCH' || project.stageLabel === 'APPROVAL') needs.push('procurement/award evidence');
  if (project.freshnessScore < 50) needs.push('fresh corroborating evidence');
  needs.push('confirmed equipment requirement before treating prediction as demand');
  return needs.join('; ');
}
...
export function buildCommercialIntelligence(projects: ProjectIntelligence[], outcomes: CommercialOutcome[], sources: CommercialSource[]) {
  ...
  const events = projects.flatMap(eventSignals).sort((a, b) => b.confidence - a.confidence).slice(0, 100);

  const contractorMap = new Map<string, { contractor: string; projectIds: Set<string>; locations: Set<string>; stages: Set<string>; equipment: Set<string>; highPriority: number; priorityTotal: number }>();
  for (const project of projects) for (const contractor of project.contractors) {
    const key = normalise(contractor);
    const item = contractorMap.get(key) || { contractor, projectIds: new Set<string>(), locations: new Set<string>(), stages: new Set<string>(), equipment: new Set<string>(), highPriority: 0, priorityTotal: 0 };
    if (!item.projectIds.has(project.id)) { item.projectIds.add(project.id); item.priorityTotal += project.bdmPriority; if (project.priorityBand === 'HIGH') item.highPriority += 1; }
    item.locations.add(project.location); item.stages.add(project.stageLabel); project.equipmentPrediction.classes.forEach(value => item.equipment.add(value)); contractorMap.set(key, item);
  }
  const contractorWorkload = [...contractorMap.values()].map(item => ({ contractor: item.contractor, projectCount: item.projectIds.size, highPriorityProjects: item.highPriority, averagePriority: item.projectIds.size ? Math.round(item.priorityTotal / item.projectIds.size) : 0, locations: [...item.locations], stages: [...item.stages], predictedEquipment: [...item.equipment] })).sort((a, b) => b.projectCount - a.projectCount || b.averagePriority - a.averagePriority).slice(0, 40);

  const clusterMap = new Map<string, { location: string; equipmentClass: string; projects: Set<string>; priorityTotal: number; confidenceTotal: number }>();
  for (const project of projects) for (const equipmentClass of project.equipmentPrediction.classes) {
    const key = `${normalise(project.location)}|${equipmentClass}`;
    const item = clusterMap.get(key) || { location: project.location, equipmentClass, projects: new Set<string>(), priorityTotal: 0, confidenceTotal: 0 };
    if (!item.projects.has(project.id)) { item.projects.add(project.id); item.priorityTotal += project.bdmPriority; item.confidenceTotal += project.equipmentPrediction.confidence; }
    clusterMap.set(key, item);
  }
  const equipmentClusters = [...clusterMap.values()].map(item => ({ label: 'PREDICTED' as const, location: item.location, equipmentClass: item.equipmentClass, projectCount: item.projects.size, averagePriority: Math.round(item.priorityTotal / item.projects.size), confidence: Math.round(item.confidenceTotal / item.projects.size), projectIds: [...item.projects] })).sort((a, b) => (b.projectCount * b.averagePriority) - (a.projectCount * a.averagePriority)).slice(0, 40);
  ...
  const scopeProgram = [
    ...
    { from: 2131, to: 2150, name: 'Equipment demand & fleet-positioning decision layer', status: 'OPERATIONAL', note: 'Regional equipment clusters and fleet-positioning watch recommendations are active and explicitly PREDICTED.' },
  ];

  return { implementedThrough: 2150, scopeCount: 150, pilotQueue, events, contractorWorkload, equipmentClusters, fleetPositioning, calibration: { linkedRealOutcomes, unmatchedRealOutcomes, sufficientSample: linkedRealOutcomes >= 20, priorityDeciles: deciles }, sourceCoverage, scopeProgram };
}
