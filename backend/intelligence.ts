...
function sourceReliability(sourceKey: string, source?: IntelligenceSource) {
  ...
  if (includesAny(key, ['qtrip', 'infrastructure', 'generation', 'connection']))
    return 0.9;
  if (includesAny(sector, ['infrastructure', 'procurement', 'rail', 'energy']))
    return 0.88;
  if (includesAny(sector, ['mining', 'resources', 'oil & gas'])) return 0.84;
  if (includesAny(sector, ['exploration', 'building', 'development']))
    return 0.8;
  return 0.74;
}
...
function classifyStage(
  ...
  const base = {
    ...
    observedAt: evidenceTimestamp(record)
      ? new Date(evidenceTimestamp(record)).toISOString()
      : '',
  ...
  if (
    ...
    return {
      ...
      label: 'COMPLETE',
      confidence: 0.94,
      reason: 'Published evidence indicates the work is complete.',
    };
  if (
    includesAny(haystack, [
      'shutdown',
    ...
    return {
      ...
      confidence: 0.92,
      reason:
        'Published evidence contains a shutdown, turnaround or outage signal.',
    };
  if (
    includesAny(haystack, [
      'maintenance',
    ...
    return {
      ...
      confidence: 0.86,
      reason:
        'Published evidence describes maintenance, repair or asset-renewal work.',
    };
  if (
    includesAny(haystack, [
      'mobilisation',
    ...
    return {
      ...
      confidence: 0.9,
      reason:
        'Published evidence contains a mobilisation or site-establishment signal.',
    };
  if (
    ...
    return {
      ...
      confidence: 0.91,
      reason:
        'The evidence is an awarded-contract disclosure or explicitly states an award.',
    };
  if (
    record.sourceKey.includes('tender') ||
    record.sourceKey.includes('procurement') ||
    includesAny(haystack, [
      'request for tender',
    ...
    return {
      ...
      label: 'PROCUREMENT',
      confidence: 0.88,
      reason: 'Published evidence indicates tendering or procurement activity.',
    };
  if (
    includesAny(haystack, [
      'construction',
    ...
    return {
      ...
      label: 'CONSTRUCTION',
      confidence: 0.82,
      reason: 'Published evidence describes construction or physical works.',
    };
  if (
    record.sourceKey.includes('environmental') ||
    record.sourceKey.includes('granted') ||
    includesAny(haystack, [
      'approval',
      'approved',
      'permit',
    ...
    return {
      ...
      confidence: 0.8,
      reason:
        'Published evidence indicates an approval, permit or granted authority.',
    };
  return {
    ...base,
    label: 'WATCH',
    confidence: 0.5,
    reason: 'Evidence confirms activity but not a later commercial stage.',
  };
}

function predictEquipment(
  records: IntelligenceEvidence[],
  ...
  const haystack = records
    .map((record) =>
      [
        ...
        record.description,
        record.sourceKey,
        sources.get(record.sourceKey)?.sector || '',
      ]
        .join(' ')
        .toLowerCase(),
    )
  ...
  if (
    ...
    ['Excavators', 'Loaders', 'Graders', 'Rollers', 'Support fleet'].forEach(
      (item) => classes.add(item),
    );
    reasons.push('civil, earthworks or mining work');
    matchedGroups += 1;
  }
  if (
    includesAny(haystack, [
    ...
    ['Excavators', 'Pumps / dewatering', 'Generators'].forEach((item) =>
      classes.add(item),
    );
    reasons.push('water, pipeline or dewatering work');
    matchedGroups += 1;
  }
  if (includesAny(haystack, ['rail', 'track', 'station'])) {
    ['Excavators', 'Telehandlers', 'Lighting towers'].forEach((item) =>
      classes.add(item),
    );
    reasons.push('rail or track work');
    matchedGroups += 1;
  }
  if (
    includesAny(haystack, [
    ...
    reasons.push('maintenance or shutdown work');
    matchedGroups += 1;
  }
  if (
    includesAny(haystack, [
    ...
    reasons.push('energy or electrical infrastructure work');
    matchedGroups += 1;
  }
  if (
    includesAny(haystack, [
    ...
    ['Access equipment', 'Telehandlers', 'Generators'].forEach((item) =>
      classes.add(item),
    );
    reasons.push('building or facilities work');
    matchedGroups += 1;
  }
  const averageReliability = records.length
    ? records.reduce(
        (sum, record) =>
          sum +
          sourceReliability(record.sourceKey, sources.get(record.sourceKey)),
        0,
      ) / records.length
    : 0;
  const confidence = classes.size
    ? Math.round(
        clamp(
          (45 + matchedGroups * 9 + Math.min(10, records.length * 2)) *
            (0.75 + averageReliability * 0.25),
          0,
          92,
        ),
      )
  ...
  return {
    ...
    reason: reasons.length
      ? 'Predicted from observed ' + reasons.join(', ') + '.'
      : 'No sufficiently specific work-type evidence to infer an equipment class.',
  };
}

function contradictionPenalty(records: IntelligenceEvidence[]) {
  const haystack = records
    .map((record) => `${record.project} ${record.description}`.toLowerCase())
    .join(' ');
  let penalty = 0;
  if (
    includesAny(haystack, ['cancelled', 'canceled', 'withdrawn', 'abandoned'])
  )
    penalty += 30;
  if (includesAny(haystack, ['deferred', 'postponed', 'on hold', 'suspended']))
    penalty += 18;
  if (
    includesAny(haystack, ['completed', 'works complete', 'project complete'])
  )
    penalty += 12;
...
function scoreSignalQuality(
  ...
  const qualityBand: SignalQualityBand =
    score >= 80 ? 'A' : score >= 65 ? 'B' : score >= 50 ? 'C' : 'D';
  return {
    reliabilityScore: Math.round(averageReliability * 100),
    freshnessScore: Math.round(newestFreshness),
...
function scoreContractorConfidence(
  records: IntelligenceEvidence[],
  sources: Map<string, IntelligenceSource>,
) {
  const named = records.filter(
    (record) =>
      record.company.trim() &&
      record.organisationRole === 'DELIVERY_CONTRACTOR',
  );
  if (!named.length) return { score: 0, band: 'LOW' as ConfidenceBand };
  const uniqueCompanies = new Set(
    named.map((record) => normaliseKey(record.company)),
  ).size;
  const reliability =
    named.reduce(
      (sum, record) =>
  ...
  const score = Math.round(
    clamp(
      40 +
        ...
        Math.max(0, uniqueCompanies - 1) * 5,
      0,
      95,
    ),
  );
...
function scorePriority(
  records: IntelligenceEvidence[],
  stage: StageSignal,
  equipment: ReturnType<typeof predictEquipment>,
  signalQuality: ReturnType<typeof scoreSignalQuality>,
) {
  const relevanceMultiplier = equipment.classes.length ? 1 : 0.4;
  const freshnessPoints = Math.round(
    signalQuality.freshnessScore * 0.25 * relevanceMultiplier,
  );
  const stagePoints: Record<StageLabel, number> = {
    WATCH: 4,
    APPROVAL: 10,
    PROCUREMENT: 18,
    AWARDED: 15,
  ...
  const equipmentRelevance = equipment.classes.length
    ? Math.min(15, 8 + equipment.classes.length)
    : 0;
  const reliabilityPoints = Math.round(signalQuality.reliabilityScore * 0.1);
  const corroborationPoints = Math.round(
    signalQuality.corroborationScore * 0.4,
  );
  const contradictionPoints = Math.round(
    signalQuality.contradictionPenalty * 0.5,
  );
  const priority = clamp(
    freshnessPoints +
      stagePoints[stage.label] +
      contractor +
      equipmentRelevance +
      reliabilityPoints +
      corroborationPoints -
  ...
  return {
    priority,
    factors: [
      `Freshness decay ${freshnessPoints}/25`,
      `Stage/timing ${stagePoints[stage.label]}/25`,
      `Contractor evidence ${contractor}/15`,
      `Equipment relevance ${equipmentRelevance}/15`,
      `Source reliability ${reliabilityPoints}/10`,
      `Independent corroboration ${corroborationPoints}/10`,
      `Contradiction penalty -${contradictionPoints}`,
    ],
  };
}
...
export async function buildProjectIntelligence(
  ...
  const snapshotByKey = new Map(
    snapshotPage.items.map((item) => [item.projectKey, item]),
  );
  ...
  for (const [projectKey, projectRecords] of groups) {
    ...
    const previous =
      ...
      (candidate.projectKey === projectKey || !groups.has(candidate.projectKey))
        ? candidate
        : undefined;
    ...
    const transitionQualified =
      ...
      currentStage.reliability >= 0.82 &&
      (uniqueSources >= 2 || currentStage.reliability >= 0.92);
    ...
    projects.push({
      ...
      contractors,
      contractorConfidence: contractorConfidence.score,
      contractorConfidenceBand: contractorConfidence.band,
      sources,
      ...
      stageConfidence: Math.round(currentStage.confidence * 100),
      stageReason: currentStage.reason,
      stageChanged: advanced,
      previousStage: advanced && previous ? previous.stageLabel : '',
      stageEvidence: `${currentStage.sourceKey} · ${currentStage.observedAt}`,
      stageTransitionQualified: transitionQualified,
      equipmentPrediction,
      sourceReliability: signalQuality.reliabilityScore,
      freshnessScore: signalQuality.freshnessScore,
      corroborationScore: signalQuality.corroborationScore,
      contradictionPenalty: signalQuality.contradictionPenalty,
      signalQualityScore: signalQuality.score,
      signalQualityBand: signalQuality.band,
      bdmPriority: priority.priority,
      priorityBand:
...
export function buildCalibrationMetrics(
  projects: ProjectIntelligence[],
  outcomes: PilotCalibrationOutcome[],
): CalibrationMetrics {
  const { groups, unmatched } = groupReviewedOutcomes(projects, outcomes);
  const bands: CalibrationBand[] = ['A', 'B', 'C', 'D'].map((value) => ({
    band: value as SignalQualityBand,
    reviewed: 0,
    confirmed: 0,
    quoted: 0,
    won: 0,
  ...
  for (const { project, results } of groups) {
    const bucket = bands.find(
      (item) => item.band === project.signalQualityBand,
    )!;
    bucket.reviewed += 1;
    if (
  ...
  return {
    matchedReviewed: groups.length,
    unmatchedReviewed: unmatched,
    bands,
  };
}
