...
it('does not clear a held revision when a preferred live duplicate lacks its restriction',async()=>{
  store.tables.opportunities=[live(1,{sourceObservedAt:'2026-09-01',description:'New current description'})];
  store.tables.evidence_pages=[{id:'held-version',sourceKey:'qa-source',events:[evidence(1,{sourceObservedAt:'2020-01-01',qualityFlags:['INVALID_INPUT'],contextOnly:true})]}];
  const before=JSON.stringify(store.tables),result=await load();
  expect(result.items).toHaveLength(1);
  expect(result.items[0].description).toBe('New current description');
  expect(result.items[0].qualityFlags).toEqual(expect.arrayContaining(['INVALID_INPUT','CONTEXT_ONLY']));
