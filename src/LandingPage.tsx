...
type PublicPage = 'home' | 'products' | 'solutions' | 'industries' | 'insights' | 'about' | 'privacy' | 'terms' | 'contact';
...
function AustraliaGraphic({ compact = false }: { compact?: boolean }) {
  const points = [[70,156],[107,111],[147,211],[188,75],[232,91],[281,105],[337,153],[352,181],[327,224],[295,252],[258,205],[216,164],[173,166],[103,194]];
  return <svg className={compact ? 'hi2-australia-svg compact' : 'hi2-australia-svg'} viewBox='0 0 420 330' role='img' aria-label='Accurate outline map of Australia'>
    <path d={MAINLAND_PATH} fill='#eef0f2' stroke='#a9b0b8' strokeWidth='2'/>
    <path d={TASMANIA_PATH} fill='#eef0f2' stroke='#a9b0b8' strokeWidth='2'/>
    {!compact && <>
      {points.map(([cx, cy], index) => <circle key={`${cx}-${cy}`} cx={cx} cy={cy} r={index % 4 === 0 ? 6 : 4} fill='#ef2029' opacity={index % 4 === 0 ? 1 : .8}/>)}
      <text x='88' y='176'>WA</text><text x='210' y='99'>NT</text><text x='207' y='211'>SA</text><text x='318' y='130'>QLD</text><text x='326' y='206'>NSW</text><text x='309' y='245'>VIC</text><text x='320' y='314'>TAS</text>
    </>}
  </svg>;
}
...
const PUBLIC_DETAILS: Record<Exclude<PublicPage, 'home'>, { eyebrow: string; title: string; copy: string; cards: Array<{ title: string; copy: string }> }> = {
  products: {
    ...
    cards: [
      { title: 'Decision Desk', copy: 'Prioritised projects, live feed health and the evidence that deserves attention today.' },
      { title: 'Commercial Intelligence', copy: 'Pilot queue, event intelligence, contractor workload, predicted demand clusters and fleet-positioning watch.' },
      ...
      { title: 'Alerts', copy: 'High-priority projects, stage movements, tender/procurement activity and contractor-linked signals.' },
      { title: 'Resources', copy: 'Resource-sector project intelligence spanning mining, petroleum, exploration and authority evidence.' },
      { title: 'Source Admin', copy: 'Live source health, deferred collectors, provenance, rights and historical backfill status.' }
    ]
  },
  solutions: {
    eyebrow: 'SOLUTIONS',
    ...
    cards: [
      { title: 'BDM prospecting', copy: 'Rank opportunities so sales teams spend time where project evidence is strongest.' },
      { title: 'Fleet planning', copy: 'Review PREDICTED regional equipment demand before committing fleet or capital.' },
      { title: 'Shutdown planning', copy: 'Surface shutdown, outage and maintenance signals early enough to prepare commercially.' },
      { title: 'Contractor tracking', copy: 'Follow evidence-backed contractor activity across active projects and regions.' },
      { title: 'Market coverage', copy: 'Bring mining, civil, energy, development and procurement signals into one governed view.' },
      { title: 'Commercial measurement', copy: 'Record real BDM outcomes and use them to calibrate priorities without synthetic performance history.' }
    ]
  },
  industries: {
    eyebrow: 'INDUSTRIES',
    ...
    cards: [
      { title: 'Mining & resources', copy: 'Tenements, exploration, approvals, mine activity, shutdowns and resource-project evidence.' },
      { title: 'Civil & infrastructure', copy: 'Roads, major works, public procurement, reconstruction and transport programmes.' },
      { title: 'Energy & power', copy: 'Generation, connection, renewable, petroleum and energy-project signals.' },
      { title: 'Rail', copy: 'Rail programme, procurement and contract-disclosure intelligence.' },
      { title: 'Water infrastructure', copy: 'Water-board, utility and infrastructure procurement signals.' },
      { title: 'Construction & development', copy: 'Building permits, development activity and major project evidence.' }
    ]
  },
  insights: {
    ...
    cards: [
      { title: 'Evidence first', copy: 'Project stages and priorities are derived from retained public-source evidence.' },
      { title: 'Predictions stay predictions', copy: 'Equipment demand is labelled PREDICTED and is not presented as an observed hire requirement.' },
      { title: 'Commercial outcomes are real', copy: 'Only human-entered BDM outcomes are used for measured commercial calibration.' },
      { title: 'Source health is explicit', copy: 'Failed or deferred collectors remain visible instead of being silently treated as live.' }
    ]
  },
  about: {
    ...
    cards: [
      { title: 'Evidence governed', copy: 'Every project traces back to retained source evidence and provenance.' },
      { title: 'No fabricated demand', copy: 'Predicted equipment demand remains clearly labelled until a real requirement is confirmed.' },
      { title: 'National ambition', copy: 'Coverage is expanded only where lawful, machine-readable sources can be validated.' },
      { title: 'Commercial feedback loop', copy: 'Real BDM outcomes can be linked to the signals that generated the opportunity.' },
      { title: 'Built for rental teams', copy: 'The product is organised around calls, fleet, contractors, projects and timing.' },
      { title: 'Continuous improvement', copy: 'Source quality, project resolution and commercial calibration improve as evidence accumulates.' }
    ]
  },
  privacy: {
    eyebrow: 'PRIVACY',
    ...
    cards: [
      { title: 'Public-source intelligence', copy: 'Project and source intelligence is built from lawful public information and retained provenance.' },
      { title: 'Commercial inputs', copy: 'BDM outcomes entered into the platform are treated as operational business records.' },
      { title: 'No invented people', copy: 'The platform does not manufacture personal contacts where evidence is unavailable.' },
      { title: 'Purpose limitation', copy: 'Information is presented for legitimate rental-market intelligence, planning and business-development workflows.' }
    ]
  },
  terms: {
    eyebrow: 'TERMS',
    ...
    cards: [
      { title: 'Verify before outreach', copy: 'Contractor, timing and actual equipment requirements should be confirmed before commercial action.' },
      { title: 'Predicted demand', copy: 'PREDICTED equipment output is an analytical inference, not a confirmed hire order.' },
      { title: 'Source availability', copy: 'Public-source access can change and collectors may be deferred when reliability or rights are unclear.' },
      { title: 'No guaranteed outcome', copy: 'Priority scores and signals support decisions; they do not guarantee project awards, hires or revenue.' }
    ]
  },
  contact: {
    eyebrow: 'CONTACT',
    ...
    cards: [
      { title: 'Platform walkthrough', copy: 'Open the live workspace and review current projects, opportunities, map intelligence and reports.' },
      { title: 'Rental-team use cases', copy: 'Review BDM prioritisation, contractor activity and predicted fleet demand.' },
      { title: 'Data coverage', copy: 'Inspect live source health, deferred collectors and historical backfill from Source Admin.' },
      { title: 'Commercial pilot', copy: 'Record genuine BDM outcomes to begin measuring signal quality against real results.' }
    ]
  }
};
    ...
    <section className='hi2-public-hero'>
      <div>
        ...
        <div className='hi2-hero-actions'>
          <button className='hi2-red-button hi2-large' onClick={onExplore}>Explore the platform <ArrowRight size={16}/></button>
          <button className='hi2-outline-button hi2-large' onClick={onDemo}>Get a demo</button>
        </div>
      </div>
      <div className='hi2-public-map'><AustraliaGraphic compact/><b>Australian rental intelligence</b><span>Evidence governed · project linked · commercially actionable</span></div>
    </section>
...
export default function LandingPage({ onExplore }: LandingPageProps) {
  const publicPageFromHash = (): PublicPage => {
    ...
    const pages: PublicPage[] = ['products','solutions','industries','insights','about','privacy','terms','contact'];
  ...
  return <div className='hi2-page'>
    <header className='hi2-header'>
      <button className='hi2-logo' onClick={() => openPublicPage('home')} aria-label='Hire Intelligence home'>
        <span className='hi2-slash'/><strong>Hire Intelligence</strong>
      </button>
      <nav className='hi2-nav' aria-label='Primary navigation'>
        <a className={publicPage === 'products' ? 'active' : ''} href='#products'>Products</a>
        <a className={publicPage === 'solutions' ? 'active' : ''} href='#solutions'>Solutions</a>
        <a className={publicPage === 'industries' ? 'active' : ''} href='#industries'>Industries</a>
        <a className={publicPage === 'insights' ? 'active' : ''} href='#insights'>Insights</a>
        <a className={publicPage === 'about' ? 'active' : ''} href='#about'>About</a>
      </nav>
      <div className='hi2-header-actions'>
        <button className='hi2-login' onClick={onExplore}>Log in</button>
        <button className='hi2-red-button' onClick={() => setDemoOpen(true)}>Get a demo <ArrowRight size={15}/></button>
      </div>
    </header>
    ...
    {publicPage === 'home' ? <main>
      <section className='hi2-hero' id='products'>
        <div className='hi2-hero-copy'>
          ...
          <p>Hire Intelligence turns complex project, contractor, shutdown and market data into clear, early signals — so you can plan, position and grow.</p>
          <div className='hi2-hero-actions'>
            <button className='hi2-red-button hi2-large' onClick={() => setDemoOpen(true)}>Get a demo <ArrowRight size={16}/></button>
            <button className='hi2-outline-button hi2-large' onClick={onExplore}>Explore the platform</button>
          </div>
        ...
        <div className='hi2-hero-art' aria-label='Hire Intelligence platform preview'>
          ...
          <div className='hi2-dashboard-frame'>
            <div className='hi2-dashboard-top'>
              <div className='hi2-dashboard-brand'><span className='hi2-mini-slash'/>Hire Intelligence</div>
              <div className='hi2-dashboard-search'>Search projects, companies, people…</div>
      ...
      <section className='hi2-features' id='solutions'>
        {FEATURES.map(feature => <article className='hi2-feature' key={feature.key}>
          ...
          <h2>{feature.title}</h2>
          <p>{feature.copy}</p>
          <button onClick={onExplore}>Learn more <ArrowRight size={14}/></button>
        </article>)}
      </section>
      ...
      <section className='hi2-proof' id='industries'>
        ...
        <div className='hi2-australia-card'>
          ...
          <div><b>BUILT FOR AUSTRALIA</b><span>Backed by local data, for a stronger hire industry.</span></div>
        </div>
      </section>
