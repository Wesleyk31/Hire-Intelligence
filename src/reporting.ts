...
export function buildExecutiveReportSummary(dashboard: DashboardLike): ReportSummary {
  ...
  const failed = (dashboard.sources?.states || []).filter((source: any) => source.status === 'FAILED').length;
  const universe = dashboard.universe || {};
  return {
    generatedAt: new Date().toISOString(),
    headline: `${projects.length} loaded canonical projects, ${events.length} current opportunity signals, ${high} high-priority projects`,
...
function regionalRows(projects: any[]) {
  ...
  for (const project of projects) {
    ...
    const row = regions.get(region) || { projects: 0, high: 0, equipment: new Set<string>() };
    row.projects += 1;
    if (project.priorityBand === 'HIGH') row.high += 1;
    for (const item of project.equipmentPrediction?.classes || []) row.equipment.add(item);
    regions.set(region, row);
  }
  return [...regions.entries()].map(([region, row]) => [region, String(row.projects), String(row.high), [...row.equipment].join(', ') || 'No class inferred']).sort((a, b) => Number(b[1]) - Number(a[1])).slice(0, 40);
}
...
export function downloadExecutivePdf(dashboard: DashboardLike) {
  ...
  doc.setFontSize(22); doc.text('Hire Intelligence', 14, 20);
  doc.setFontSize(15); doc.text('Executive Intelligence Report', 14, 29);
  doc.setFontSize(9); doc.text(`Generated ${new Date(summary.generatedAt).toLocaleString('en-AU')}`, 14, 36);
  doc.text('Evidence-governed equipment-rental opportunity intelligence', 14, 42);
  autoTable(doc, {
    startY: 50,
    ...
    body: [
      ...
      ['Historical evidence processed', summary.evidenceProcessed.toLocaleString('en-AU')],
  ...
  doc.setFontSize(10); doc.text('Decision interpretation', 14, finalY + 9);
  doc.setFontSize(8);
  doc.text(doc.splitTextToSize('CALL NOW requires a strong current project signal, evidence-backed delivery contractor, sufficiently fresh evidence and a meaningful predicted equipment class. PREDICTED equipment is an analytical inference and is not a confirmed hire requirement. Users must verify contractor, timing and equipment requirement before outreach or fleet movement.', 180), 14, finalY + 15);

  section(doc, 'CALL NOW Queue', 'Highest-confidence commercially actionable records meeting the evidence gate.');
  autoTable(doc, { startY: 30, head: [['Project', 'Location', 'Stage', 'Priority', 'Contractor', 'PREDICTED equipment']], body: callNow.slice(0, 30).map((project: any) => [project.name, project.location, project.stageLabel, String(project.bdmPriority), (project.contractors || []).join(', ') || 'Not evidenced', (project.equipmentPrediction?.classes || []).join(', ') || 'Not inferred']), styles: { fontSize: 6.8 } });

  section(doc, 'Top Ranked Projects', 'Canonical projects consolidated from retained evidence.');
  autoTable(doc, { startY: 30, head: [['Project', 'Location', 'Stage', 'Priority', 'Signal', 'PREDICTED equipment', 'Evidence']], body: projects.slice(0, 50).map((project: any) => [project.name, project.location, project.stageLabel, String(project.bdmPriority), project.signalQualityBand || '-', (project.equipmentPrediction?.classes || []).join(', ') || 'Not inferred', String(project.evidenceCount || 0)]), styles: { fontSize: 6.2, cellPadding: 1.4 } });

  section(doc, 'Regional Intelligence', 'Unique project counts by current location label; state-level and approximate map records remain disclosed in the platform.');
  autoTable(doc, { startY: 30, head: [['Region / location', 'Projects', 'High priority', 'PREDICTED equipment classes']], body: regionalRows(projects), styles: { fontSize: 6.5 } });

  section(doc, 'Current Opportunity Signals');
  autoTable(doc, { startY: 25, head: [['Signal', 'Project', 'Location', 'Confidence', 'Stage', 'Recommended action']], body: events.slice(0, 60).map((event: any) => [event.type, event.project, event.location, `${event.confidence}%`, event.stage || '-', event.action || event.reason || '-']), styles: { fontSize: 6.2, cellPadding: 1.4 } });

  section(doc, 'Delivery Organisations and Workload', 'Only organisations classified as delivery contractors should appear as contractors; owners/applicants/operators are separated in the platform.');
  autoTable(doc, { startY: 30, head: [['Delivery organisation', 'Projects', 'High priority', 'Avg priority', 'Locations', 'PREDICTED equipment']], body: contractors.slice(0, 50).map((item: any) => [item.contractor, String(item.projectCount), String(item.highPriorityProjects), String(item.averagePriority), (item.locations || []).join(', '), (item.predictedEquipment || []).join(', ') || 'Not inferred']), styles: { fontSize: 6.2 } });
